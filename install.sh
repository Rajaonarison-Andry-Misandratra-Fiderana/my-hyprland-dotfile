...
setup_system() {
	...
	# Force initialize and bypass unstable keyservers by fetching directly via curl
	sudo pacman-key --init
	echo "🔑 Fetching Chaotic-AUR key directly..."
	curl -sL "https://repo.chaotic.cx/chaotic-keyring.gpg" | sudo pacman-key --add -

	sudo pacman-key --lsign-key 3056513887B78AEB

	# Installation of Keyring and Mirrorlist
	sudo pacman -U --noconfirm \
		'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-keyring.pkg.tar.zst' \
		'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-mirrorlist.pkg.tar.zst'

	# Inject into pacman.conf if missing
...
apply_configs() {
	...
	if [ -d "./assets" ]; then
		...
		cp -rf ./assets/* "$HOME/Pictures/Wallpapers/"
	fi
